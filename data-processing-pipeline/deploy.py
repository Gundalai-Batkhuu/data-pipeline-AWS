...
class KedroStepFunctionsStack(core.Stack):
    ...
    ecr_repository_name = "dpp_step_functions_ecr"
    s3_raw_data_bucket_name = (
        "raw-data-7374046"  # this is where the raw data is located
    )
    s3_intermediate_data_bucket_name = (
        "intermediate-data-7374046"  # this is where the intermediate data is located
    )
    s3_processed_data_bucket_name = (
        "processed-data-7374046"  # this is where the intermediate data is located
    )
    ...
    def __init__(self, scope: core.Construct, id: str, **kwargs) -> None:
        ...
        self._set_ecr_image()
        self._set_s3_raw_data_bucket()
        self._set_s3_intermediate_data_bucket()
        self._set_s3_processed_data_bucket()
    ...
    def _set_ecr_repository(self) -> None:
        ...
        self.ecr_repository = aws_ecr.Repository.from_repository_name(
            self, id="dpp_step_functions_ecr", repository_name=self.ecr_repository_name
        )
    ...
    def _set_s3_raw_data_bucket(self) -> None:
        """Set the S3 bucket containing the raw data"""
        self.s3_raw_bucket = s3.Bucket.from_bucket_name(
            self, "RawDataBucket", bucket_name=self.s3_raw_data_bucket_name
        )

    def _set_s3_intermediate_data_bucket(self) -> None:
        """Set the S3 bucket containing the intermediate data"""
        self.s3_intermediate_bucket = s3.Bucket(
            self, "IntermediateDataBucket", bucket_name=self.s3_intermediate_data_bucket_name
        )

    def _set_s3_processed_data_bucket(self) -> None:
        """Set the S3 bucket containing the processed data"""
        self.s3_processed_bucket = s3.Bucket.from_bucket_name(
            self, "ProcessedDataBucket", bucket_name=self.s3_processed_data_bucket_name
        )
    ...
    def _convert_kedro_node_to_lambda_function(self, node: Node) -> IFunction:
        ...
        func = aws_lambda.Function(
            ...
            description=str(node.name),                 # Changed into .name
            code=self.ecr_image,
        ...
        self.s3_raw_bucket.grant_read_write(func)
        self.s3_intermediate_bucket.grant_read_write(func)
        self.s3_processed_bucket.grant_read_write(func)
        return func
